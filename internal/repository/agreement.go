...
func NewAgreementRepository() *AgreementRepository {
	ar := &AgreementRepository{
		...
		mySQLManager: database.NewMySqlManager(),
	}
	ar.rm.Connect()
	ar.mySQLManager.Connect()
	return ar
}
...
func (ar *AgreementRepository) GetAgreement(key uint64) (*model.Agreement, error) {
	...
	agreement := model.Agreement{}
	err := database.Query(ar.mySQLManager, &agreement, "SELECT * FROM agreement WHERE id = ?", strconv.FormatUint(key, 10))
